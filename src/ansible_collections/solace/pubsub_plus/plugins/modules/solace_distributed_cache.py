...
short_description: distributed cache on a message vpn
description:
- "Allows addition, removal and configuration of Distributed Cache Objects on a Message Vpn in an idempotent manner."
notes:
...
class SolaceDistributedCacheTask(SolaceBrokerCRUDTask):
    ...
    def get_func(self, vpn_name, cache_name):
        # GET /msgVpns/{msgVpnName}/distributedCaches/{cacheName}
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG, 'msgVpns',
                      vpn_name, 'distributedCaches', cache_name]
        return self.sempv2_api.get_object_settings(self.get_config(), path_array)

    def create_func(self, vpn_name, cache_name, settings=None):
        # POST /msgVpns/{msgVpnName}/distributedCaches
        data = {
            'msgVpnName': vpn_name,
            self.OBJECT_KEY: cache_name
        }
        data.update(settings if settings else {})
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name, 'distributedCaches']
        return self.sempv2_api.make_post_request(self.get_config(), path_array, data)

    def update_func(self, vpn_name, cache_name, settings=None, delta_settings=None):
        # PATCH /msgVpns/{msgVpnName}/distributedCaches/{cacheName}
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG, 'msgVpns',
                      vpn_name, 'distributedCaches', cache_name]
        return self.sempv2_api.make_patch_request(self.get_config(), path_array, settings)

    def delete_func(self, vpn_name, cache_name):
        # DELETE /msgVpns/{msgVpnName}/distributedCaches/{cacheName}
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG, 'msgVpns',
                      vpn_name, 'distributedCaches', cache_name]
...
def run_module():
    module_args = dict(
        name=dict(type='str', required=True, aliases=['cache', 'cache_name'])
    )
