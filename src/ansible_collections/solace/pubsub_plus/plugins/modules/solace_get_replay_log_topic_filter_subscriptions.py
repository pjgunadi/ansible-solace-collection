...
DOCUMENTATION = '''
---
module: solace_get_replay_log_topic_filter_subscriptions
short_description: get list of topic filter subscriptions on a replay log
description:
- "Get a list of Topic Filter Subscription objects configured on a Replay Log object."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_replay_log_topic_filter_subscriptions:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetReplayLogTopicFilterSubscriptionsTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        ...
        return ['msgVpns', params['msg_vpn'], 'replayLogs',
                params['replay_log_name'], 'topicFilterSubscriptions']
...
def run_module():
    ...
    solace_task = SolaceGetReplayLogTopicFilterSubscriptionsTask(module)
    solace_task.execute()
