...
DOCUMENTATION = '''
---
module: solace_get_authentication_oauth_profile_resource_server_required_claims
short_description: get list of resource server required claims on an oauth profile
description:
- "Get a list of Resource Server Required Claim objects configured on an Authentication OAuth Profile."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_authentication_oauth_profile_resource_server_required_claims:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetAuthenticationOAuthProfileResourceServerRequiredClaimsTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        ...
        return ['msgVpns', params['msg_vpn'], 'authenticationOauthProfiles',
                params['oauth_profile_name'], 'resourceServerRequiredClaims']
...
def run_module():
    ...
    solace_task = SolaceGetAuthenticationOAuthProfileResourceServerRequiredClaimsTask(module)
    solace_task.execute()
