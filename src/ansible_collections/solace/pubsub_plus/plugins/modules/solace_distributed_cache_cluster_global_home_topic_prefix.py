...
DOCUMENTATION = '''
---
module: solace_distributed_cache_cluster_global_home_topic_prefix
short_description: topic prefix on a global caching home cluster
description:
- "Allows addition and removal of Topic Prefix Objects on a Global Caching Home Cluster."
notes:
...
options:
  name:
    description: The topic prefix. Maps to 'topicPrefix' in the API.
    required: true
    type: str
    aliases: [topic_prefix]
  home_cluster_name:
...
- solace.pubsub_plus
module_defaults:
  solace_distributed_cache_cluster_global_home_topic_prefix:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceDistributedCacheClusterGlobalHomeTopicPrefixTask(SolaceBrokerCRUDTask):
    ...
    def get_args(self):
        ...
        return [params['msg_vpn'], params['cache_name'], params['cluster_name'],
                params['home_cluster_name'], params['name']]

    def get_func(self, vpn_name, cache_name, cluster_name, home_cluster_name, topic_prefix):
        # GET .../globalCachingHomeClusters/{homeClusterName}/topicPrefixes/{topicPrefix}
...
def run_module():
    module_args = dict(
        name=dict(type='str', required=True, aliases=['topic_prefix']),
    ...
    solace_task = SolaceDistributedCacheClusterGlobalHomeTopicPrefixTask(module)
    solace_task.execute()
