...
DOCUMENTATION = '''
---
module: solace_get_dmr_cluster_cert_matching_rule_attribute_filters
short_description: get list of attribute filters on a dmr cluster certificate matching rule
description:
- "Get a list of Attribute Filter objects configured on a DMR Cluster Certificate Matching Rule object."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_dmr_cluster_cert_matching_rule_attribute_filters:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
- name: get list config
  solace_get_dmr_cluster_cert_matching_rule_attribute_filters:
    ...
    cert_matching_rule_name: "{{ cert_matching_rule_name }}"
  register: result
...
class SolaceGetDmrClusterCertMatchingRuleAttributeFiltersTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        # GET /dmrClusters/{dmrClusterName}/certMatchingRules/{certMatchingRuleName}/attributeFilters
        return ['dmrClusters', params['dmr_cluster_name'], 'certMatchingRules',
                params['cert_matching_rule_name'], 'attributeFilters']
...
def run_module():
    ...
    solace_task = SolaceGetDmrClusterCertMatchingRuleAttributeFiltersTask(module)
    solace_task.execute()
