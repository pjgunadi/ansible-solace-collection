...
DOCUMENTATION = '''
---
module: solace_get_authentication_oauth_profile_client_required_claims
short_description: get list of client required claims on an oauth profile
description:
- "Get a list of Client Required Claim objects configured on an Authentication OAuth Profile."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_authentication_oauth_profile_client_required_claims:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetAuthenticationOAuthProfileClientRequiredClaimsTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        # GET /msgVpns/{msgVpnName}/authenticationOauthProfiles/{oauthProfileName}/clientRequiredClaims
        return ['msgVpns', params['msg_vpn'], 'authenticationOauthProfiles',
                params['oauth_profile_name'], 'clientRequiredClaims']
...
def run_module():
    ...
    solace_task = SolaceGetAuthenticationOAuthProfileClientRequiredClaimsTask(module)
    solace_task.execute()
