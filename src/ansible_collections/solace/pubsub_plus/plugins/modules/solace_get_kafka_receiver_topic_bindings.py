...
DOCUMENTATION = '''
---
module: solace_get_kafka_receiver_topic_bindings
short_description: get list of topic bindings on a kafka receiver
description:
- "Get a list of Topic Binding objects configured on a Kafka Receiver object."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_kafka_receiver_topic_bindings:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetKafkaReceiverTopicBindingsTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        # GET /msgVpns/{msgVpnName}/kafkaReceivers/{kafkaReceiverName}/topicBindings
        return ['msgVpns', params['msg_vpn'], 'kafkaReceivers',
                params['kafka_receiver_name'], 'topicBindings']
...
def run_module():
    ...
    solace_task = SolaceGetKafkaReceiverTopicBindingsTask(module)
    solace_task.execute()
