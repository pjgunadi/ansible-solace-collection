...
DOCUMENTATION = '''
---
module: solace_get_kafka_sender_queue_bindings
short_description: get list of queue bindings on a kafka sender
description:
- "Get a list of Queue Binding objects configured on a Kafka Sender object."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_kafka_sender_queue_bindings:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetKafkaSenderQueueBindingsTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        # GET /msgVpns/{msgVpnName}/kafkaSenders/{kafkaSenderName}/queueBindings
        return ['msgVpns', params['msg_vpn'], 'kafkaSenders',
                params['kafka_sender_name'], 'queueBindings']
...
def run_module():
    ...
    solace_task = SolaceGetKafkaSenderQueueBindingsTask(module)
    solace_task.execute()
