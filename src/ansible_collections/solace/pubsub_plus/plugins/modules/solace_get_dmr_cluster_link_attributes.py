...
DOCUMENTATION = '''
---
module: solace_get_dmr_cluster_link_attributes
short_description: get list of attributes on a dmr cluster link
description:
- "Get a list of Attribute objects configured on a DMR Cluster link object."
notes:
...
seealso:
- module: solace.pubsub_plus.solace_dmr_cluster_link_attribute
author:
...
- solace.pubsub_plus
module_defaults:
  solace_get_dmr_cluster_link_attributes:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetDmrClusterLinkAttributesTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        # GET /dmrClusters/{dmrClusterName}/links/{remoteNodeName}/attributes
        return ['dmrClusters', params['dmr_cluster_name'], 'links', params['remote_node_name'], 'attributes']
...
def run_module():
    ...
    solace_task = SolaceGetDmrClusterLinkAttributesTask(module)
    solace_task.execute()
