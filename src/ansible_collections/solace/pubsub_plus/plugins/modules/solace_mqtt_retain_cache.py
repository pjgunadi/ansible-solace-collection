...
DOCUMENTATION = '''
---
module: solace_mqtt_retain_cache
short_description: mqtt retain cache on a message vpn
description:
- "Allows addition, removal and configuration of MQTT Retain Cache Objects on a Message Vpn in an idempotent manner."
notes:
...
seealso:
- module: solace.pubsub_plus.solace_get_mqtt_retain_caches
author:
...
class SolaceMqttRetainCacheTask(SolaceBrokerCRUDTask):
    ...
    def create_func(self, vpn_name, mqtt_retain_cache_name, settings=None):
        # POST /msgVpns/{msgVpnName}/mqttRetainCaches
        data = {
            'msgVpnName': vpn_name,
            self.OBJECT_KEY: mqtt_retain_cache_name
        }
        data.update(settings if settings else {})
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG,
                      'msgVpns', vpn_name, 'mqttRetainCaches']
...
def run_module():
    module_args = dict(
        name=dict(type='str', required=True, aliases=['mqtt_retain_cache', 'mqtt_retain_cache_name'])
    )
    ...
    solace_task = SolaceMqttRetainCacheTask(module)
    solace_task.execute()
