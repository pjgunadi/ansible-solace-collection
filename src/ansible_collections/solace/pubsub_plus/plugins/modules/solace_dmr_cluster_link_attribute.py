...
DOCUMENTATION = '''
---
module: solace_dmr_cluster_link_attribute
short_description: attribute for dmr cluster link
description:
- "Allows addition, removal and configuration of Attribute Objects on DMR Cluster links."
...
- module: solace.pubsub_plus.solace_get_dmr_cluster_link_attributes
author:
...
- solace.pubsub_plus
module_defaults:
  solace_dmr_cluster_link_attribute:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceDmrClusterLinkAttributeTask(SolaceBrokerCRUDTask):
    ...
    def get_args(self):
        ...
        return [params['dmr_cluster_name'], params['remote_node_name'], params['name'], params['value']]

    def get_func(self, dmr_cluster_name, remote_node_name, attribute_name, attribute_value):
        # GET /dmrClusters/{dmrClusterName}/links/{remoteNodeName}/attributes/{attributeName},{attributeValue}
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG, 'dmrClusters', dmr_cluster_name,
                      'links', remote_node_name, 'attributes', f"{attribute_name},{attribute_value}"]
        return self.sempv2_api.get_object_settings(self.get_config(), path_array)

    def create_func(self, dmr_cluster_name, remote_node_name, attribute_name, attribute_value, settings=None):
        # POST /dmrClusters/{dmrClusterName}/links/{remoteNodeName}/attributes
        data = {
        ...
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG, 'dmrClusters',
                      dmr_cluster_name, 'links', remote_node_name, 'attributes']
        return self.sempv2_api.make_post_request(self.get_config(), path_array, data)

    def delete_func(self, dmr_cluster_name, remote_node_name, attribute_name, attribute_value):
        # DELETE /dmrClusters/{dmrClusterName}/links/{remoteNodeName}/attributes/{attributeName},{attributeValue}
        path_array = [SolaceSempV2Api.API_BASE_SEMPV2_CONFIG, 'dmrClusters', dmr_cluster_name,
                      'links', remote_node_name, 'attributes', f"{attribute_name},{attribute_value}"]
...
def run_module():
    ...
    solace_task = SolaceDmrClusterLinkAttributeTask(module)
    solace_task.execute()
