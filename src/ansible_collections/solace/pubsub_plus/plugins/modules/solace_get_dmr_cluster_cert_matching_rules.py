...
DOCUMENTATION = '''
---
module: solace_get_dmr_cluster_cert_matching_rules
short_description: get list of certificate matching rules on a dmr cluster
description:
- "Get a list of Certificate Matching Rule objects configured on a DMR Cluster object."
notes:
...
- solace.pubsub_plus
module_defaults:
  solace_get_dmr_cluster_cert_matching_rules:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetDmrClusterCertMatchingRulesTask(SolaceBrokerGetPagingTask):
...
def run_module():
    ...
    solace_task = SolaceGetDmrClusterCertMatchingRulesTask(module)
    solace_task.execute()
