...
DOCUMENTATION = '''
---
module: solace_get_rdp_rest_consumer_oauth_jwt_claims
short_description: get list of oauth jwt claims on a rdp rest consumer
description:
- "Get a list of OAuth JWT Claim objects configured on a RDP REST Consumer object."
notes:
...
seealso:
- module: solace.pubsub_plus.solace_rdp_rest_consumer_oauth_jwt_claim
author:
...
- solace.pubsub_plus
module_defaults:
  solace_get_rdp_rest_consumer_oauth_jwt_claims:
    host: "{{ sempv2_host }}"
    port: "{{ sempv2_port }}"
...
class SolaceGetRdpRestConsumerOauthJwtClaimsTask(SolaceBrokerGetPagingTask):
    ...
    def get_path_array(self, params: dict) -> list:
        # GET /msgVpns/{msgVpnName}/restDeliveryPoints/{restDeliveryPointName}/restConsumers/{restConsumerName}/oauthJwtClaims
        return ['msgVpns', params['msg_vpn'], 'restDeliveryPoints', params['rdp_name'],
                'restConsumers', params['rest_consumer_name'], 'oauthJwtClaims']
...
def run_module():
    ...
    solace_task = SolaceGetRdpRestConsumerOauthJwtClaimsTask(module)
    solace_task.execute()
